...
export default class CustomEffectsHandler {
    ...
    async createNewCustomEffect() {
        ...
        //@ts-ignore
        const effects = await item.createEmbeddedDocuments("ActiveEffect", [newEffect]);
    ...
    async editCustomEffect(effect) {
        ...
        const activeEffect = item.effects.find((activeEffect) => activeEffect.id === effect.id);
    ...
    _findCustomEffectsItem() {
        ...
        return game.items?.get(game.settings.get(CONSTANTS.MODULE_ID, "customEffectsItemId"));
    }
