import { debugM, errorM, i18n, isStringEquals, logM, warnM } from "./effect-utility.js";
import FoundryHelpers from "./foundry-helpers.js";
import Effect from "../effects-public/effect.js";
import { EffectSupport } from "./effect-support.js";
import API from "../api.js";
import { warn } from "../lib/lib.js";
export default class EffectGenericHandler {
    ...
    async toggleEffect(effectName, overlay, uuids, metadata = undefined, effectData = undefined) {
        debugM(this.moduleName, `START Effect Handler 'toggleEffect' : [effectName=${effectName},overlay=${overlay},uuids=${String(uuids)},metadata=${String(metadata)}]`);
        const effectNames = [];
        ...
        debugM(this.moduleName, `END Effect Handler 'toggleEffect' : [effectName=${effectName},overlay=${overlay},effectNames=${String(effectNames)},metadata=${String(metadata)}]`);
        return true;
    }
    /**
     * Toggles an effect on or off by name on an actor by UUID
    ...
    async addEffect(effectName, effectData, uuid, origin, overlay = false, metadata = undefined) {
        const actor = this._foundryHelpers.getActorByUuid(uuid);
        let effect = this._findEffectByName(effectName, actor);
        ...
        effect.overlay =
            String(effectData.overlay) === "false" || String(effectData.overlay) === "true"
                ...
                : String(overlay) === "false" || String(overlay) === "true"
                    ? overlay
                    : false;
        const activeEffectFounded = await this.findEffectByNameOnActor(effectName, uuid);
        ...
        if (await API.hasNestedEffects(activeEffectData)) {
            activeEffectData = await API._getNestedEffectSelection(activeEffectData);
            if (!activeEffectData) {
     ...
     */
    _findEffectByName(effectName, actor) {
        if (!effectName) {
            return undefined;
        }
        let effect;
    ...
    hasEffectAppliedFromIdOnActor(effectId, uuid, includeDisabled = false) {
        ...
        const isApplied = actorEffects.some((activeEffect) => {
            if (includeDisabled) {
                ...
                //@ts-ignore
                if (activeEffect?.id === effectId && !activeEffect.disabled) {
                    return true;
                }
                else {
                    return false;
    ...
    async removeEffectOnActor(effectName, uuid) {
        ...
        //@ts-ignore
        const actorEffects = actor?.effects || [];
        const effectToRemove = actorEffects.find((activeEffect) => activeEffect?.label === effectName);
        if (!effectToRemove) {
            debugM(this.moduleName, `Can't find effect to remove with name ${effectName} from ${actor.name} - ${actor.id}`);
            return undefined;
        }
    ...
    async removeEffectFromIdOnActor(effectId, uuid) {
        ...
        if (effectId) {
            ...
            const effectToRemove = (actorEffects.find((activeEffect) => activeEffect.id === effectId));
            if (!effectToRemove) {
    ...
    async addEffectOnActor(effectName, uuid, origin, overlay, effect) {
        debugM(this.moduleName, `START Effect Handler 'addEffectOnActor' : [effectName=${effectName},uuid=${uuid},origin=${origin},overlay=${overlay},effect=${effect}]`);
        if (effectName) {
            effectName = i18n(effectName);
        }
        if (effect) {
            ...
            const activeEffectsAdded = (await actor.createEmbeddedDocuments("ActiveEffect", [activeEffectData]));
            logM(this.moduleName, `Added effect ${effect.label ? effect.label : effectName} to ${actor.name} - ${actor.id}`);
            return activeEffectsAdded[0];
        }
    ...
    async toggleEffectFromIdOnActor(effectId, uuid, alwaysDelete, forceEnabled, forceDisabled, overlay) {
        ...
        //@ts-ignore
        const actorEffects = actor?.effects || [];
        const activeEffect = actorEffects.find((entity) => {
     ...
     * @param {boolean} overlay - if the effect is an overlay or not
     */
    async addActiveEffectOnActor(uuid, activeEffectData, overlay = false) {
        if (activeEffectData) {
            const actor = this._foundryHelpers.getActorByUuid(uuid);
            //@ts-ignore
    ...
    async removeEffectOnToken(effectName, uuid) {
        ...
        const activeEffectsRemoved = (await token.actor?.deleteEmbeddedDocuments("ActiveEffect", [effectToRemove.id]));
    ...
    async removeEffectFromIdOnToken(effectId, uuid) {
        debugM(this.moduleName, `START Effect Handler 'removeEffectFromIdOnToken' : [effectId=${effectId},uuid=${uuid}]`);
        if (effectId) {
            const token = this._foundryHelpers.getTokenByUuid(uuid);
            const actorEffects = 
            //@ts-ignore
            token.actor?.effects?.contents || [];
            const effectToRemove = actorEffects.find(
            //@ts-ignore
    ...
    async removeEffectFromIdOnTokenMultiple(effectIds, uuid) {
        if (effectIds) {
            ...
            for (const effectIdTmp of effectIds) {
                const effectToRemove = actorEffects.find((activeEffect) => activeEffect?._id === effectIdTmp);
                if (effectToRemove) {
                    effectIdsTmp.push(effectIdTmp);
                }
            }
            const activeEffectsRemoved = (await token.actor?.deleteEmbeddedDocuments("ActiveEffect", effectIdsTmp));
    ...
    async addEffectOnToken(effectName, uuid, origin, overlay, effect) {
        ...
        if (effect) {
            ...
            effect.overlay =
                String(effect.overlay) === "false" || String(effect.overlay) === "true"
                    ...
                    : String(overlay) === "false" || String(overlay) === "true"
                        ? overlay
                        : false;
            const activeEffectFounded = await this.findEffectByNameOnToken(effectName, uuid);
            if (activeEffectFounded) {
            ...
            const activeEffectsAdded = (await token.actor?.createEmbeddedDocuments("ActiveEffect", [activeEffectData]));
            logM(this.moduleName, `Added effect ${effect.label ? effect.label : effectName} to ${token.name} - ${token.id}`);
            return activeEffectsAdded[0];
        }
        warnM(this.moduleName, `No effect object is been passed`);
        return undefined;
    ...
    async toggleEffectFromIdOnToken(effectId, uuid, alwaysDelete, forceEnabled, forceDisabled, overlay) {
        debugM(this.moduleName, `START Effect Handler 'toggleEffectFromIdOnToken' : [effectId=${effectId},uuid=${uuid},alwaysDelete=${alwaysDelete},forceEnabled=${forceEnabled},forceDisabled=${forceDisabled},overlay=${overlay}]`);
        const token = this._foundryHelpers.getTokenByUuid(uuid);
        //@ts-ignore
        const actorEffects = token.actor?.effects?.contents || [];
        const activeEffect = actorEffects.find(
        //@ts-ignore
        (activeEffect) => activeEffect?._id === effectId);
        if (!activeEffect) {
            return undefined;
        ...
        debugM(this.moduleName, `END Effect Handler 'toggleEffectFromIdOnToken' : [effectName=${activeEffect.name},tokenName=${token.name},alwaysDelete=${alwaysDelete},forceEnabled=${forceEnabled},forceDisabled=${forceDisabled},overlay=${overlay}]`);
        return !!updated;
    }
    /**
     * @href https://github.com/ElfFriend-DnD/foundryvtt-temp-effects-as-statuses/blob/main/scripts/temp-effects-as-statuses.js
    ...
    async toggleEffectFromDataOnToken(effect, uuid, alwaysDelete, forceEnabled, forceDisabled, overlay) {
        debugM(this.moduleName, `START Effect Handler 'toggleEffectFromIdOnToken' : [effect=${effect},uuid=${uuid},alwaysDelete=${alwaysDelete},forceEnabled=${forceEnabled},forceDisabled=${forceDisabled},overlay=${overlay}]`);
        const token = this._foundryHelpers.getTokenByUuid(uuid);
        //@ts-ignore
        const actorEffects = token.actor?.effects?.contents || [];
        const activeEffect = actorEffects.find((activeEffect) => {
            return (
            //@ts-ignore
            isStringEquals(activeEffect?._id, effect.customId) ||
                //@ts-ignore
                isStringEquals(activeEffect?.label, effect.name));
        });
        if (!activeEffect) {
            return undefined;
        }
     ...
     * @param {boolean} overlay - if the effect is an overlay or not
     */
    async addActiveEffectOnToken(uuid, activeEffectData, overlay = false) {
        debugM(this.moduleName, `START Effect Handler 'addActiveEffectOnToken' : [uuid=${uuid},activeEffectData=${activeEffectData}]`);
        if (activeEffectData) {
            const token = this._foundryHelpers.getTokenByUuid(uuid);
            //@ts-ignore
            if (!activeEffectData.origin) {
            ...
            //@ts-ignore
            activeEffectData.flags = foundry.utils.mergeObject(activeEffectData.flags, coreFlags);
            const activeEffetsAdded = (await token.actor?.createEmbeddedDocuments("ActiveEffect", [activeEffectData]));
            //@ts-ignore
            logM(this.moduleName, `Added effect ${activeEffectData.label} to ${token.name} - ${token.id}`);
            debugM(this.moduleName, `END Effect Handler 'addActiveEffectOnToken' : [uuid=${uuid},activeEffectData=${activeEffectData}]`);
    ...
    async updateEffectFromIdOnToken(effectId, uuid, origin, overlay, effectUpdated) {
        debugM(this.moduleName, `START Effect Handler 'updateEffectFromIdOnToken' : [effectId=${effectId}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        const token = this._foundryHelpers.getTokenByUuid(uuid);
        //@ts-ignore
        const actorEffects = token.actor?.effects?.contents || [];
        const activeEffect = actorEffects.find((activeEffect) => activeEffect?._id === effectId);
        if (!activeEffect) {
            return undefined;
        }
        if (!origin) {
        ...
        if (await API.hasNestedEffects(activeEffectDataUpdated)) {
            activeEffectDataUpdated = await API._getNestedEffectSelection(activeEffectDataUpdated);
        ...
        debugM(this.moduleName, `END Effect Handler 'updateEffectFromIdOnToken' : [effectId=${effectId}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        return !!updated;
    }
    ...
    async updateEffectFromNameOnToken(effectName, uuid, origin, overlay, effectUpdated) {
        debugM(this.moduleName, `START Effect Handler 'updateEffectFromNameOnToken' : [effectName=${effectName}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        const token = this._foundryHelpers.getTokenByUuid(uuid);
        //@ts-ignore
        const actorEffects = token.actor?.effects?.contents || [];
        const activeEffect = actorEffects.find((activeEffect) => isStringEquals(activeEffect?.label, effectName));
        if (!activeEffect) {
            return undefined;
        }
        if (!origin) {
        ...
        //@ts-ignore
        logM(this.moduleName, `Updated effect ${activeEffect.label} to ${token.name} - ${token.id}`);
        debugM(this.moduleName, `END Effect Handler 'updateEffectFromNameOnToken' : [effectName=${effectName}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        return !!updated;
    }
    ...
    async updateActiveEffectFromIdOnToken(effectId, uuid, origin, overlay, effectUpdated) {
        debugM(this.moduleName, `START Effect Handler 'updateActiveEffectFromIdOnToken' : [effectId=${effectId}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        const token = this._foundryHelpers.getTokenByUuid(uuid);
        //@ts-ignore
        const actorEffects = token.actor?.effects?.contents || [];
        const activeEffect = actorEffects.find((activeEffect) => activeEffect?._id === effectId);
        ...
        //@ts-ignore
        logM(this.moduleName, `Updated effect ${activeEffect.label} to ${token.name} - ${token.id}`);
        debugM(this.moduleName, `END Effect Handler 'updateActiveEffectFromIdOnToken' : [effectId=${effectId}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        return !!updated;
    }
    ...
    async updateActiveEffectFromNameOnToken(effectName, uuid, origin, overlay, effectUpdated) {
        debugM(this.moduleName, `START Effect Handler 'updateActiveEffectFromNameOnToken' : [effectName=${effectName}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        const token = this._foundryHelpers.getTokenByUuid(uuid);
        //@ts-ignore
        const actorEffects = token.actor?.effects?.contents || [];
        const activeEffect = actorEffects.find((activeEffect) => isStringEquals(activeEffect?.label, effectName));
        ...
        //@ts-ignore
        logM(this.moduleName, `Updated effect ${activeEffect.label} to ${token.name} - ${token.id}`);
        debugM(this.moduleName, `END Effect Handler 'updateActiveEffectFromNameOnToken' : [effectName=${effectName}, uuid=${uuid}, origin=${origin}, overlay=${overlay}, effectUpdated=${effectUpdated}]`);
        return !!updated;
    }
    ...
    async onManageActiveEffectFromEffectId(effectActions, owner, effectId, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled) {
        debugM(this.moduleName, `START Effect Handler 'onManageActiveEffectFromEffectId' : [effectActions=${effectActions}, owner=${owner}, effectId=${effectId},
        ...
        const activeEffect = actorEffects.find((activeEffect) => activeEffect?._id === effectId);
        const response = await this.onManageActiveEffectFromActiveEffect(effectActions, owner, activeEffect, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled);
        debugM(this.moduleName, `END Effect Handler 'onManageActiveEffectFromEffectId' : [effectActions=${effectActions}, owner=${owner}, effectId=${effectId},
        alwaysDelete=${alwaysDelete}, forceEnabled=${forceEnabled}, forceDisabled=${forceDisabled}, isTemporary=${isTemporary},
        isDisabled=${isDisabled}]`);
        return response;
    }
    async onManageActiveEffectFromEffectIdArr(...inAttributes) {
        ...
        return this.onManageActiveEffectFromEffectId(effectActions, owner, effectId, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled);
    }
    /**
     * Manage Active Effect instances through the Actor Sheet via effect control buttons.
     * @param {MouseEvent} event      The left-click event on the effect control
    ...
    async onManageActiveEffectFromEffect(effectActions, owner, effect, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled) {
        debugM(this.moduleName, `START Effect Handler 'onManageActiveEffectFromEffect' : [effectActions=${effectActions}, owner=${owner}, effect=${effect},
        alwaysDelete=${alwaysDelete}, forceEnabled=${forceEnabled}, forceDisabled=${forceDisabled}, isTemporary=${isTemporary},
        isDisabled=${isDisabled}]`);
        const activeEffect = effect.label ? owner.effects.getName(i18n(effect.label)) : null;
        const response = await this.onManageActiveEffectFromActiveEffect(effectActions, owner, activeEffect, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled);
        debugM(this.moduleName, `END Effect Handler 'onManageActiveEffectFromEffect' : [effectActions=${effectActions}, owner=${owner}, effect=${effect},
        alwaysDelete=${alwaysDelete}, forceEnabled=${forceEnabled}, forceDisabled=${forceDisabled}, isTemporary=${isTemporary},
        isDisabled=${isDisabled}]`);
        return response;
    }
    async onManageActiveEffectFromEffectArr(...inAttributes) {
        ...
        return this.onManageActiveEffectFromEffect(effectActions, owner, effect, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled);
    }
    /**
     * Manage Active Effect instances through the Actor Sheet via effect control buttons.
     * @param {MouseEvent} event      The left-click event on the effect control
    ...
    async onManageActiveEffectFromActiveEffect(effectActions, owner, activeEffect, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled) {
        debugM(this.moduleName, `START Effect Handler 'onManageActiveEffectFromActiveEffect' : [effectActions=${effectActions}, owner=${owner}, activeEffect=${activeEffect},
        alwaysDelete=${alwaysDelete}, forceEnabled=${forceEnabled}, forceDisabled=${forceDisabled}, isTemporary=${isTemporary},
        isDisabled=${isDisabled}]`);
        switch (effectActions) {
            case "update": {
                ...
                if (owner instanceof Actor) {
                    ...
                    const activeEffectsUpdated = (await actor?.updateEmbeddedDocuments("ActiveEffect", [activeEffect]));
            ...
            case "create": {
                ...
                if (owner instanceof Actor) {
                    ...
                    const activeEffectsAdded = (await actor?.createEmbeddedDocuments("ActiveEffect", [activeEffect]));
                    return activeEffectsAdded[0];
                }
                else if (owner instanceof Item) {
                    const item = owner;
            ...
            case "delete": {
                ...
                const activeEffectDeleted = await activeEffect?.delete();
    ...
    async onManageActiveEffectFromActiveEffectArr(...inAttributes) {
        ...
        return await this.onManageActiveEffectFromActiveEffect(effectActions, owner, activeEffect, alwaysDelete, forceEnabled, forceDisabled, isTemporary, isDisabled);
    }
}
