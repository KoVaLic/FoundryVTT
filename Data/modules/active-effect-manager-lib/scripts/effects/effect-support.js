...
import Effect, { Constants } from "../effects-public/effect.js";
import { isStringEquals, is_real_number } from "./effect-utility.js";
import "../constants.js";
export class EffectSupport {
    static buildDefault(
    // effectData: Effect,
    id, name, icon, isPassive, changes = [], atlChanges = [], tokenMagicChanges = [], atcvChanges = []) {
        return new Effect({
            customId: id,
            name: i18n(name),
            description: ``,
    ...
    static isDuplicateEffectChange(aeKey, arrChanges) {
    ...
    static _handleIntegrations(effect) {
        const arrChanges = [];
    ...
    static _getDurationData(seconds, rounds, turns, isTemporary) {
    ...
    */
    ...
    static _getCombatRounds(seconds, rounds) {
        if (rounds) {
            return rounds;
        }
        if (seconds) {
    ...
    static _getSeconds(seconds, rounds) {
        if (seconds) {
            return seconds;
        }
        if (rounds) {
    ...
    static convertActiveEffectToEffect(activeEffect) {
        //@ts-ignore
        ...
        //@ts-ignore
        const changes = activeEffect.changes.filter((change) => !change.key.startsWith("ATL") && change.key !== "macro.tokenMagic" && !change.key.startsWith("ATCV"));
        //@ts-ignore
        ...
        const convenientDescription = hasProperty(activeEffect, `flags.${Constants.MODULE_ID}.${Constants.FLAGS.DESCRIPTION}`)
        ...
        return new Effect({
            ...
            //@ts-ignore
            description: i18n(activeEffect.flags.customEffectDescription),
            //@ts-ignore
    ...
    static convertActiveEffectDataPropertiesToActiveEffect(p, isPassive) {
        ...
        return ActiveEffect.create({
            ...
            icon: p.icon,
            tint: p.tint,
            duration: EffectSupport._getDurationData(p.duration.seconds, p.duration.rounds, p.duration.turns, !isPassive),
            ...
            origin: origin ? origin : p.origin ? p.origin : "",
     ...
     */
    static convertToActiveEffectData(effect) {
        ...
        return {
            ...
            description: i18n(effect.description),
            icon: effect.icon,
            tint: effect.tint,
            ...
            origin: effect.origin ? effect.origin : "None",
     ...
     * @returns {object} The active effect data object for this effect
     */
    static convertToActiveEffect(effect) {
    ...
    //   return changes;
    // }
    static retrieveChangesOrderedByPriorityFromAE(activeEffect) {
        // Organize non-disabled effects by their application priority
        const changes = [activeEffect].reduce((changes, e) => {
            //@ts-ignore
            if (e.disabled) {
                return changes;
            }
    ...
    height = null, width = null, scale = null, alpha = null) {
        ...
        const effectNameI18 = i18n(effectName);
        ...
        const efffectAtlToApply = new Effect({
            ...
            icon: effectIcon,
            tint: "",
            seconds: duration !== null ? duration * 60 : undefined,
            rounds: NaN,
            turns: NaN,
            isDynamic: false,
            isViewable: true,
    ...
    static prepareOriginForToken(tokenOrTokenId) {
        let token = undefined;
        if (typeof tokenOrTokenId === "string" || tokenOrTokenId instanceof String) {
            const tokens = canvas.tokens?.placeables;
            token = tokens.find((token) => token.id === tokenOrTokenId);
        }
    ...
    static prepareOriginForActor(actorOrActorId) {
        let actor = undefined;
        if (typeof actorOrActorId === "string" || actorOrActorId instanceof String) {
            actor = game.actors?.get(actorOrActorId);
        }
    ...
    static prepareOriginForItem(itemOrItemId) {
    ...
    static prepareOriginFromEntity(entity) {
        let origin = "None";
        if (!entity) {
            return origin;
        }
        if (typeof entity === "string" || entity instanceof String) {
            origin = EffectSupport.prepareOriginForToken(entity);
            if (!origin || origin === "None") {
                origin = EffectSupport.prepareOriginForActor(entity);
            }
        }
