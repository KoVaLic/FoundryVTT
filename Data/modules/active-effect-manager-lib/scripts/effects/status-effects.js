...
import { isStringEquals } from "./effect-utility.js";
...
export default class StatusEffectsLib {
  ...
  */
    ...
    _fetchStatusEffects(statusEffectNames) {
    ...
    async onToggleEffect({ token, wrapper, args }) {
        ...
        if (statusEffectId) {
            ...
            if (!effect) {
                ...
                const statusEffectInternal = arrayStatusEffects.find((e) => {
                ...
                if (statusEffectInternal) {
                    let activeEffectFound = undefined;
                    ...
                    if (statusEffectId.startsWith("Convenient Effect:")) {
                        ...
                        activeEffectFound = await API.findEffectByNameOnToken(tokenId, effectName);
                    }
                    else {
                        const effectId = statusEffectId;
                        activeEffectFound = await API.findEffectByIdOnToken(tokenId, effectId);
                    }
                    if (!activeEffectFound) {
                        for (const effect of Object.values(token.actor.effects.contents)) {
                            if (isStringEquals(
                            //@ts-ignore
                            effect.flags.core.statusId, "Convenient Effect: " + statusEffectInternal.label)) {
                    ...
                    if (activeEffectFound) {
                        //@ts-ignore
                        const effectName = activeEffectFound.label;
                        ...
                        */
                        ...
                        const effectId = activeEffectFound.id;
    ...
    _getStatusEffectChoicesInternal(token) {
        // const token = args[0]; //<Token>(<unknown>this);
        ...
        let effectsArray = 
        //@ts-ignore
        token.actor?.effects?.contents || doc.effects || [];
        ...
        const tokenEffects = foundry.utils.deepClone(effectsArray) || [];
        //@ts-ignore
        ...
        // });
        const statusEffectDefaultFiltered = tokenEffects.filter((e) => {
            let result = true;
            for (const efct of CONFIG.statusEffects) {
                if (isStringEquals(e.label, efct.label)) {
                    result = false;
                    break;
                }
            }
        ...
        return (CONFIG.statusEffects.concat(statusEffectDefaultFiltered).reduce((obj, e) => {
            if (e.contents) {
                let activeEffects = e.contents;
                if (game.settings.get(CONSTANTS.MODULE_NAME, "showOnlyTemporaryStatusEffectNames")) {
                ...
                for (const activeEffect of activeEffects) {
                    if (activeEffect) {
                        //@ts-ignore
                        let labelEffect = activeEffect.label || "";
                        labelEffect = labelEffect.replace("Convenient Effect:", "").trim();
                        //@ts-ignore
                        const iconEffect = activeEffect.icon || "";
                        //@ts-ignore
     ...
     */
    getStatusEffectChoices(token, wrapped, ...args) {
