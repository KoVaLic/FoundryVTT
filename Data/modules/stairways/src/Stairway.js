...
export class Stairway extends PlaceableObject {
  ...
  get labelTextStyle () {
    ...
    const color = this.document.textColor ? foundry.utils.Color.fromString(this.document.textColor) : new foundry.utils.Color(0xFFFFFF)
    const hsv = color.hsv
    style.fill = color
  ...
  async _draw () {
    ...
    this.controlIcon = this.addChild(new StairwayControlIcon({ label: this.label, textStyle: this.labelTextStyle, texture: this.icon, width: this.width, height: this.height }))
  ...
  async _refresh (options) {
    ...
    this.line.visible = this.layer.active
    this.lockIcon.visible = this.layer.active && this.document.disabled === true
  ...
  updateOtherPlaceable () {
    ...
    if (this.otherPlaceable === false) {
      ...
      const others = canvas.stairways.placeables.filter((other) => other.onScene && !other._original && this.name === other.name && this !== other)
...
// TODO: wait for https://github.com/tc39/proposal-static-class-features
Stairway.embeddedName = 'Stairway'
