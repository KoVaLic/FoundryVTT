...
export class StairwayControl extends PIXI.Container {
  ...
  async draw () {
    ...
    // label
    this.label = this.label || this.addChild(new PreciseText(this.stairway.label, this.stairway.labelText))
  ...
  get isVisible () {
    ...
    return canvas.effects.visibility.testVisibility(point, { tolerance: 2, object: this })
  }
  ...
  async _onMouseDown (event) {
    ...
    if (targetSceneId !== null) {
      ...
      const tokenData = selectedTokens.map(token => {
        return {
          _id: token.id,
          x: Math.roundFast(targetData.x - token.w / 2),
          y: Math.roundFast(targetData.y - token.h / 2)
        }
      })
